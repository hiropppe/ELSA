...
model_vec = "tweet_to_w.txt" # the word embedding file 
out_dict = "jp_vocab.json" # target vocabulary file
out_vec = "jp_wv" # numpy verison of pretrained word embedding
with open(model_vec, "r") as stream:
    for i, line in enumerate(stream):
        if i > 0:
            data = line.strip().split(' ')
            if data[0] not in tmp_vocab:
                tmp_vocab.append(data[0])
print(tmp_vocab[0])
